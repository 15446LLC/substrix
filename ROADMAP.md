...
1. **Undeposited Funds Hygiene** — promoted to top priority (2026-06-19). Payments sitting in Undeposited Funds instead of matched to a deposit is one of the most common QBO data-quality issues, and it misstates cash/balance sheet. Confirmed via live debug endpoint (`/api/debug-undeposited-funds`) that the bank-rec `cleared` field (Reconciled/Cleared/Uncleared) does **not** track deposit-matching status for this account — every Payment line, including ones from 2011, comes back `Uncleared` via the GeneralLedger report regardless of the `R` checkmark shown in QBO's own Undeposited Funds register. The `R` in the QBO UI is a separate signal (Payment matched to a Deposit via `LinkedTxn`), not the `cleared` enum. Detection requires querying `Payment` entities directly and checking `LinkedTxn` for a linked `Deposit` — no Deposit link means the payment is still unswept.
...
3. **Cross-Report Consistency Checks** — sanity checks like "does the balance sheet balance," "does P&L net income tie to retained earnings movement," "does AR aging total match the AR balance sheet figure." Unlike per-account checks, this catches systemic data corruption that no single check would surface.
4. **Uncategorized / Suspense Transactions** — directly misstates P&L line items; common, often large $ amounts sitting unclassified.
5. **Chart of Accounts Quality** — mis-mapped accounts corrupt every report built on top of them; root-cause level.
6. **Duplicate Transactions** — duplicate vendor bills/expenses or duplicate bank-feed entries directly overstate expenses/liabilities; common and easy to detect.
7. **Journal Entry Quality** — manual JEs bypass normal transaction controls, can introduce unbalanced or erroneous entries straight into reports.

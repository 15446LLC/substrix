...
const LOGO_PATH = path.join(__dirname, '../public/assets/substrix-logo-180.png');
...
async function generatePdfReport(session, stream) {
  ...
  // --- Title block ---
  try {
    doc.image(LOGO_PATH, MARGIN, w.y - 2, { width: 46 });
  } catch (err) {
    console.error('PDF logo missing, rendering without it:', err.message);
  }
  const textX = MARGIN + 58;
  doc.font('Helvetica-Bold').fontSize(20).fillColor(COLORS.text)
    .text('Substrix', textX, w.y);
  doc.font('Helvetica').fontSize(11).fillColor(COLORS.muted)
    .text('Books Health Report', textX, w.y + 24);
  const today = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  doc.fontSize(9).fillColor(COLORS.muted)
    .text(`${companyInfo?.CompanyName || 'QuickBooks Company'}  ·  ${today}`, textX, w.y + 40);
  w.y += 58;

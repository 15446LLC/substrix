...
async function buildReconciliationHealth(session) {
  ...
  const results = await Promise.all(
    accounts.map(async (account) => {
      ...
      try {
        const [reconciledReport, unclearedReport] = await Promise.all([
          fetchTransactionListByCleared(session, account.Id, 'Reconciled'),
          fetchTransactionListByCleared(session, account.Id, 'UnCleared'),
        ]);

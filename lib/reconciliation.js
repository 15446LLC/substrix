const { fetchAccounts, fetchTransactionListByCleared } = require('./qbo');
...
async function buildAccountHealth(session, account) {
  ...
  try {
    const reconciledReport = await fetchTransactionListByCleared(session, account.Id, 'Reconciled');
    const unclearedReport = await fetchTransactionListByCleared(session, account.Id, 'Uncleared');
    ...
    const allUncleared = parseRows(unclearedReport);
    unreconciledTxns = lastRecDate
      ? allUncleared.filter(t => t.date && t.date <= lastRecDate)
      : [];
  } catch (err) {

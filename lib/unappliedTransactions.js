const { fetchFullHistory, fetchAgedReceivableDetail, fetchAgedPayableDetail, QboAuthExpiredError } = require('./qbo');
...
async function buildUnappliedTransactionsHealth(session) {
  ...
  // Aging reports may be permission-gated on some QBO plans (5020 errors seen
  // on other report endpoints) — degrade to entity-only checks rather than
  // failing the whole module. Auth expiry still propagates so the user gets
  // the reconnect prompt.
  const tolerateAgingFailure = (promise, label) =>
    promise.catch(err => {
      if (err instanceof QboAuthExpiredError) throw err;
      console.error(`${label} aging report unavailable, skipping overpayment check:`, err.message);
      return null;
    });
  ...
  const [payments, creditMemos, vendorCredits, arAging, apAging] = await Promise.all([
    ...
    tolerateAgingFailure(fetchAgedReceivableDetail(session), 'A/R'),
    tolerateAgingFailure(fetchAgedPayableDetail(session), 'A/P'),
  ]);
  ...
  const customerOverpayments = arAging
    ? parseAgingOverpayments(arAging, 'cust_name', ['Payment', 'Credit Memo'])
    : [];
  ...
  const overpayments = apAging
    ? parseAgingOverpayments(apAging, 'vend_name', ['Vendor Credit'])
    : [];

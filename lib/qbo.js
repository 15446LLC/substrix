...
// Intuit rotates refresh tokens, so parallel fetches must not each attempt
// their own refresh — the second refresh would use an already-rotated (dead)
// refresh token and fail. Coalesce concurrent refreshes per realm.
...
async function fetchAgedReceivableDetail(session) {
  const token = await getValidToken(session);
  const today = new Date().toISOString().slice(0, 10);
  const path = `/v3/company/${session.realmId}/reports/AgedReceivableDetail`
    + `?report_date=${today}`
    + `&minorversion=65`;
  return qboGet(path, token);
}

async function fetchAgedPayableDetail(session) {
  const token = await getValidToken(session);
  const today = new Date().toISOString().slice(0, 10);
  const path = `/v3/company/${session.realmId}/reports/AgedPayableDetail`
    + `?report_date=${today}`
    + `&minorversion=65`;
  return qboGet(path, token);
}

async function fetchCompanyInfo(session) {
  const token = await getValidToken(session);
  const data = await qboGet(
    `/v3/company/${session.realmId}/companyinfo/${session.realmId}?minorversion=65`,
    token
  );
...
module.exports = {
  ...
  fetchAgedReceivableDetail,
  fetchAgedPayableDetail,
  fetchCompanyInfo,
  fetchAccountByName,

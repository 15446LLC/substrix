...
// Fetches Payment entities with TxnDate in [startDate, endDate), paginated
// sequentially (to avoid QBO throttling). DepositToAccountRef isn't queryable,
// so callers must filter by account client-side.
async function fetchPaymentsByDateRange(session, startDate, endDate, maxPages = 20) {
  const token = await getValidToken(session);
  ...
  for (let page = 0; page < maxPages; page++) {
    ...
    const query = encodeURIComponent(
      `SELECT * FROM Payment WHERE ${dateFilter} ORDERBY TxnDate DESC STARTPOSITION ${startPosition} MAXRESULTS 1000`
    );
    const data = await qboGet(
      `/v3/company/${session.realmId}/query?query=${query}&minorversion=65`,
      token
